...
const Componcopy = function ({ vale, gogo }) {
    console.log(`vale ${vale}`)
    var [num, setnum] = useState(0);

    const is_id = useSelector((state) => state.auth.array_list_2)
    const dispatch = useDispatch();
    const good_list = useSelector((state) => state.auth.array_list_2)
    ...
    const onpress = () => {
        ...
        if (tr == true) {
            ...
            dispatch(authAction.setingredient_0(vale.kr))
            setnum(2)
            return (
            ...
            console.log('여기안ㅁ?')
            dispatch(authAction.setingredient_1(vale.kr))
        }
        else if (num == 1) {
            console.log('TD')
            console.log(vale.kr)
